 ...
 */
@Service("go_chess")
public class GoChessService extends BaseService {
    @Autowired
    private Common common;
    @Autowired
    private ChessInfoDao chessInfoDao;

    private static final Logger logger = LogManager.getLogger(GoChessService.class);
    ...
    public void service(Context context) throws Exception {
        ...
        boolean isCode = true;
        if (common.isNumber(codeOrAlias)) {
            isCode = false;
        }
        String url;
        if (isCode) {
            ...
            if (aBoolean) {
                url = "/chess?" + "n_t=" + System.currentTimeMillis() + "&ex=" + common.getUUID() + "&ex2=" + common.getUUID() + "&code=" + codeOrAlias;
            ...
            url = "/chess/watch?" + "n_t=" + System.currentTimeMillis() + "&ex=" + common.getUUID() + "&ex2=" + common.getUUID() + "&alias=" + codeOrAlias;
        }
        context.setResult(Result.success(url));
