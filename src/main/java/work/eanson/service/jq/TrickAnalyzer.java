package work.eanson.service.jq;

import work.eanson.exception.PosErrorException;
...
public class TrickAnalyzer {
    ...
    public void analyze(Trick trick) {
        ...
        try {
            ...
            if (action.length() <= 3 && action.length() > 1) {
                ...
                //检测棋盘是否满了
                if (common.isCbFull(sb.toString(), width)) {
                    //提子
                    sb = new StringBuilder(common.getAfterFcCenterChess(sb.toString(), width));
                    //重置平台让黑子行棋
    ...
    private int getTargetIndex(String first, int width) {
        ...
        if (!Character.isUpperCase(rowschar)) {
            try {
                throw new PosErrorException(MsgCenter.CHESS_MOVE_ERROR_POS_ERROR);
